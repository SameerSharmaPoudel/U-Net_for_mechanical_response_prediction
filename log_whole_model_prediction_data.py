...
def log_whole_model_pred_image_data(epoch,path,image,decoded,target,train=True):
    """
    stores input microstructure images (original_binary_images), predicted images from whole model (decoded_images), ground truth images (targets) in hdf5 format for both training and validation phases 
    """ 
    filename = os.path.join(path, 'whole_model_prediction.h5')
    F = h5py.File(filename, 'a')
    decoded = decoded.detach()
    
    if train==True:
            
        group1 = F.require_group('whole_model_pred_training_image_data')
    ...
    if train==False:
        ...
        group4 = F.require_group('whole_model_pred_validation_image_data')
    ...
    F.close()
    print('Whole Model Prediction Image Data Stored !!')
	
	
def log_whole_model_pred_loss_and_Rsquare_score(training_mse_loss, val_mse_loss, training_r_square_xx, validation_r_square_xx, training_r_square_xy, validation_r_square_xy, path):
    """
    stores training and validation losses, and r-squared score training and validation accuracies as lists for the prediction from whole model in hdf5 format 
    """
    filename = os.path.join(path, 'whole_model_prediction.h5')
    F = h5py.File(filename, 'a')
    	
    group2 = F.require_group('whole_model_pred_training_loss')    
    ...
    group3 = F.require_group('whole_model_pred_training_r_square_score')    
    ...
    group5 = F.require_group('whole_model_pred_validation_loss')    
    ...
    group6 = F.require_group('whole_model_pred_validation_r_square_score')    
    ...
    F.close()	
    print('Whole Model Prediction Loss and R_square Score Stored!!!')