...
from log_decoder_prediction_data import log_dec_pred_image_data
from eval_metrics import compute_r_square
from model_utils import roll_data

def training_decoder(epoch, model, path, device, loss, optimizer):
    ...
    for data in dataloader_xy.train_dataloader:
        ...
        print(decoded_image_xx.shape)
        print(decoded_image_xy.shape)
    ...
    print(f'Epoch:{epoch}, Decoder Training Loss:{training_loss:.4f}')
    
    return training_r_square, training_loss, log_dec_pred_image_data(epoch,path,image,decoded,target,train=True)


def validation_decoder(epoch, model, path, device, loss, optimizer):
    ...
    with torch.no_grad():
        ...
        print(f'Epoch:{epoch}, Decoder Validation Loss:{validation_loss:.4f}')
        
        return validation_r_square, validation_loss, log_dec_pred_image_data(epoch,path,image,decoded,target,train=False)


def test_decoder(epoch, model, path, device, loss, optimizer):
